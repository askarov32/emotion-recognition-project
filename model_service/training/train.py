...
class EmotionModel(nn.Module):
    ...
    def forward(self, x):
        # x shape: (B, M, T) -> Добавляем канал -> (B, 1, M, T)
        x = x.unsqueeze(1)
        x = self.cnn(x)
        
        # Подготовка к LSTM: (B, C, F, T) -> (B, T, C*F)
        B, C, F, T = x.shape
...
def load_data():
    file_paths = []
    labels = []
    print("[INFO] Проверка кэша спектрограмм на диске (если кэша нет, он будет создан один раз)...")
    for label_str, label_idx in META["label2idx"].items():
        folder = os.path.join(DATA_DIR, label_str)
        if not os.path.exists(folder):
            print(f"  Внимание: папка {folder} не найдена!")
            continue
        ...
        for i, f in enumerate(files):
            ...
            # Если спектрограмма еще не была конвертирована и сохранена на диск
            if not os.path.exists(pt_path):
                feat = extract_log_mel(f, META)
                torch.save(feat, pt_path) # Сохраняем на жесткий диск рядом с .wav
...
def main():
    ...
    print(f"[INFO] Всего найдено файлов (тензоров .pt): {len(file_paths)}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(file_paths, labels_np)):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            print(f"Epoch {epoch:02d} | Train Loss: {total_loss/len(train_loader):.4f} | Val Loss: {val_loss/len(val_loader):.4f} | Val Acc: {acc:.4f}")
            
            scheduler.step(acc)
            
            if acc > fold_best_acc:
                fold_best_acc = acc
                # Если это лучшая точность из всех фолдов - сохраняем модель
